	...
	async.waterfall([
		function (done) {
			var Query = [
				...
				{
					$sort : { 'tokenowners_current.tokenOwner' : 1 }
				},
				{
					$lookup:
					{
					...
					if (err) {
						...
						if (resp.length > 0) {
							...
							if (resp && resp.length > 0) {
								//   console.log("respbidssss",resp);
								//   console.log("tokencurraddress",ReqBody.curAddr.toString());
								var IndexVal = resp.findIndex(val => val.tokenBidAddress.toString() == ReqBody.curAddr.toString());
								if (IndexVal > -1) {
...
export const IpfsMetadata = async (req, res) => {
	...
	var newmetadata = {
		...
		description: "AIDICRAFT Beta Testing Token Description"
	}
