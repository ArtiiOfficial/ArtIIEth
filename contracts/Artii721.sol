...
constructor(
    ...
    function salewithToken(string memory tokenAss,address from , uint256 tokenId, uint256 amount, string memory nft) public{
        newtokenasbid(tokenAss,from,owner(),amount,tokenId, nft);
    ...
    function newtokenasbid(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId, string memory nft) internal {
    ...
    function saleToken(
        ...
        _saleToken(from, tokenId, amount, nft);
        if (checkOrder[tokenId][from] == true) {
            delete order_place[from][tokenId];
            checkOrder[tokenId][from] = false;
        }
    ...
    mapping (uint256 => uint256) public checkMicro;
    function _saleToken(
        address payable from,
        uint256 tokenId,
        uint256 amount,
    ...
    mapping(uint256 => bool) public _checkadmin;
    function claim(uint256 slot, address tokenAddress, uint256 token_id, uint256 tslot) public{
        ERC20 t = ERC20(tokenAddress);
        uint256 data = checkMicro[token_id].div(tslot);
        if(msg.sender == owner()){
            address payable user = payable(msg.sender);
            if(t.balanceOf(msg.sender) > 0){
                require(t.balanceOf(msg.sender) == slot.sub(1), "Not Eligible to claim");
                t.transferFrom(msg.sender, address(this), slot.sub(1));
                user.transfer(data.mul(slot));
            }
            else{
                require(_checkadmin[token_id] == false, "Your Claim is Already Completed");
                _checkadmin[token_id] = true;
                user.transfer(data);
            }
        }
        else{
    ...
    function safeWithdraw(uint256 val) public onlyOwner{
