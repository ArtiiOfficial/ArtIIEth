...
const columns = [
...

  {
    ...
    <div>{record.image !="" && <img width="25" height="25" src={`${config.API}/images/${record.id}/${record.image}`}/>}</div>
...

  {
    ...
    cell: record =>
    // //console.lo(record.date,"date")
    <div>{record.coverimage !="" && <img width="25" height="25" src={`${config.API}/images/coverimage/${record.id}/${record.coverimage}`}/>}</div>
...
async function deleteR(id) {
  if (id != '') {
    ...
    setTimeout(
      () => window.location.href = "/iitraminda/UsersList",
      1000);
