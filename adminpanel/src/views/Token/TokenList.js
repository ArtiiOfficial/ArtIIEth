...
const columns = [
  ...
  },
  {
    ...
    sortable: false,
    cell:val=>
    <div>{val.tokenOwnerDb&&val.tokenOwnerDb.tokenOwner?String(val.tokenOwnerDb.tokenOwner):String(val.tokenOwner)}</div>
  ...
  
  {
    ...
    sortable: false,
    cell:val=>
    <div>{val.tokenOwnerDb&&val.tokenOwnerDb.balance>0?String(val.tokenOwnerDb.balance):String(val.balance)}</div>
  ...
  
  {
    ...
    sortable: false,
    cell:val=>
    <div>{val.tokenOwnerDb&&val.tokenOwnerDb.tokenPrice>0?String(val.tokenOwnerDb.tokenPrice):String(val.tokenPrice)}</div>
...

  {
    ...
    cell: record => {
      var dataOwner=""
      console.log(record.tokenOwnerDb.tokenOwner, 'recordssssss');
     if(record.tokenOwnerDb.tokenOwner){
       dataOwner=record.tokenOwnerDb.tokenCounts
     }
     else{
      dataOwner=record.tokenCounts
     }
...
const ViewTokens=async(id)=>{
  // alert(id)
  if (id != '') {
    window.location.href = "/iitraminda/ViewToken/" + id;
  }
}
...
export default function EnhancedTable() {
  ...
  const Addtoken = async () => {
    window.location = "/iitraminda/AddToken";
  }
...
{
  userdet && <Paper className={classes.paper}>
