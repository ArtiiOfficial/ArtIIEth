...
const columns = [
  ...
  const configdata = {
   ...
   function pageChange(pageData) {
     ...
     function editR(id) 
    {
        ...
        if(id!='') {
            window.location.href="/iitraminda/EditCategory/"+id;
        }
  ...
  async  function deleteR (id) {
       ...
       const {error} = await deletecategory(id);
           ...
           setTimeout(
        ()=> window.location.href="/iitraminda/categorylist",
        1000);
  ...
  const emptyRows = rowsPerPage - Math.min(rowsPerPage, rows.length - page * rowsPerPage);
   ...
   useEffect(() => {
    ...
    const Addcategory =async () => {
        window.location="/iitraminda/AddCategory";
    }
   ...
   const AddcategoryDet=async()=>{
    window.location="/iitraminda/AddCategorydetails";
   }
